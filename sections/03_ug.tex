\section{Introducing UG}\label{sec:ug}
...
\subsection{Design}
%
$\UG$ opts for a conservative construction while providing ease of
...
\begin{definition}[\ug] \label{def:ug}
    %
    Let $\nominalgroup = \nominalgroupexpanded$ be a nominal group, let
    $\kem$ be a key encapsulation mechanism and let $H$ be a hash function.
    %
    We define the \ug \kem as depicted in \autoref{fig:ug}.
    %
    \begin{gamefigure}{\ug Framework.}{fig:ug}
        \begin{games}{2}
    ...
    %
    The correctness of the UG framework follows from the correctness of the
    \kem used to instantiate \ug.
    %
    Formally, if \ug is instantiated with a $\delta$-correct \kem, then the
    correctness of \ug is defined as follows:
    %
    \[
