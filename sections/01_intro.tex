...
The Internet Research Task Force (IRTF)'s Crypto Forum Research Group (CFRG) is working towards finalizing a recommendation for specific constructions of hybrid KEMs. The current document~\cite{irtf-cfrg-hybrid-kems-05}

\paragraph{Our Contributions.} In this paper, we introduce and analyze $\HQB$
(Heavy Quantum Bomber), a generic hybrid KEM framework that combines a
...
the KDF input, $\HQB$ makes no special assumptions about its component schemes
...
We provide two complementary proofs that $\HQB$ is IND-CCA secure. In the
...
$\HQB$. This holds even if the post-quantum KEM is broken (without
...
is $\HQB$. We model the KDF as a pseudorandom function, keyed by the
post-quantum shared secret, so breaking $\HQB$ requires breaking the
post-quantum KEM. This holds even if the nominal group SDH security is
completely broken.

Notably, $\HQB$ does not require ciphertext second-preimage resistance
(C2PRI)\cite{CiC:BCDKSV24} from its components— SDH and IND-CCA security
suffices, nor that the classical component be an IND-CCA KEM. Thus, $\HQB$
...
by leveraging additional assumptions. $\HQB$ represents a conservative point in
...
Notation and definitions are introduced \autoref{sec:prelims}.  The $\HQB$
framework is introduced in \autoref{sec:hqb}; our generic construction --
named Heavy Quantum Bomber -- is a general framework that can be instantiated
with different component algorithms.  In \autoref{sec:hqb-proofs} we prove
